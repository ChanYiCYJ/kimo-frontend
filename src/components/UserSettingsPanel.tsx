...
function Toggle({ on, onClick, label }: { on: boolean; onClick: () => void; label: string }) {
  return (
    <button onClick={onClick} className="flex w-full items-center justify-between rounded-xl border border-gray-100 px-3.5 py-3 dark:border-gray-800" role="switch" aria-checked={on}>
      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{label}</span>
      <span className={`relative h-6 w-11 shrink-0 rounded-full transition ${on ? 'bg-gray-900 dark:bg-gray-200' : 'bg-gray-300 dark:bg-gray-700'}`}>
...
export function UserSettingsPanel({
  open, onClose, pageId, canManage, hasCustom, botName,
  ttsOn, onToggleTts, webSearchOn, onToggleWebSearch,
  onExportAll, onImport, onOpenDoc, onCustomSaved, allowCustomApi = true,
    ...
    <div className="fixed inset-0 z-[95]">
      ...
      <div className="absolute inset-y-0 right-0 flex w-full max-w-sm flex-col overflow-hidden bg-white shadow-2xl dark:bg-gray-900">
        <div className="flex shrink-0 items-center justify-between border-b border-gray-100 px-4 py-3 dark:border-gray-700">
          <div>
            <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">用户设置</h3>
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-4">
          ...
          <section className="space-y-2">
            ...
            <Toggle on={webSearchOn} onClick={onToggleWebSearch} label="网络搜索" />
          </section>
          ...
          <section className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={onExportAll} className="flex items-center justify-center gap-1.5 rounded-xl border border-gray-200 px-3 py-2.5 text-sm text-gray-600 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300">
              ...
              <button onClick={onImport} className="flex items-center justify-center gap-1.5 rounded-xl border border-gray-200 px-3 py-2.5 text-sm text-gray-600 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300">
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M7.5 12a4.5 4.5 0 109 0m-4.5-9v13.5" /></svg>
                导入会话
              </button>
            </div>
          </section>
          ...
          <section className="space-y-2">
            ...
            {canManage ? (
              ...
              <div className="space-y-2 rounded-xl border border-gray-100 p-3 dark:border-gray-800">
                <input value={cfg.endpoint} onChange={(e) => setCfg({ ...cfg, endpoint: e.target.value })} placeholder="接口地址（留空用默认）" className={inputCls} />
                <div className="relative">
                  <input value={cfg.apiKey} onChange={(e) => setCfg({ ...cfg, apiKey: e.target.value })} type={showKey ? 'text' : 'password'} placeholder="API Key（留空用默认）" className={inputCls} />
                  <button onClick={() => setShowKey((v) => !v)} className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-gray-400 hover:text-gray-600" aria-label="显示/隐藏">
                    {showKey ? '🙈' : '👁️'}
                  </button>
                </div>
                ...
                {hasCustom && <p className="text-[11px] text-indigo-500 dark:text-indigo-400">✓ 自定义 API 已启用（次数/冷却限制已解除）</p>}
              </div>
            )}
          </section>
          ...
          <section className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={onOpenDoc} className="flex items-center justify-center gap-1.5 rounded-xl border border-gray-200 px-3 py-2.5 text-sm text-gray-600 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300">
              ...
              <a href="https://github.com/ChanYiCYJ/kimo-frontend" target="_blank" rel="noreferrer" className="flex items-center justify-center gap-1.5 rounded-xl border border-gray-200 px-3 py-2.5 text-sm text-gray-600 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300">
          ...
          <p className="text-center text-[11px] text-gray-400 dark:text-gray-500">AI 生成内容仅供参考</p>
        </div>
      </div>
    </div>,
    document.body,
