...
export function AgentPanel({ onClose, onInsertMessage, onOpenKb, onExport, onArticle, onUploadFile, canExport, canArticle }: AgentPanelProps) {
  ...
  const onDrop = useCallback((e: React.DragEvent) => {
    ...
    r.onload = () => { const t = String(r.result || ""); const nm = f.name.replace(/\.(md|markdown|txt)$/, ""); const nx = [{ name: nm, content: t, createdAt: Date.now() }, ...prompts]; setPrompts(nx); S(nx); setActivePrompt(nx[0]); setTab("prompts"); };
    r.readAsText(f);
  }, [prompts]);
