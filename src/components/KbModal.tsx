    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
      ...
      <div className="relative flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900">
        ...
        <div className="flex shrink-0 gap-1 border-b border-gray-100 px-4 py-2 dark:border-gray-700">
          {(['role', 'site', 'notes'] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
          ...
          {tab === 'role' ? (
            <div className="space-y-4">
              <div className="rounded-xl border border-gray-100 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-800/50">
                ...
                <pre className="max-h-48 overflow-y-auto whitespace-pre-wrap text-xs leading-relaxed text-gray-600 dark:text-gray-300">{systemPrompt || '（未设置）'}</pre>
              </div>
              {promptPreview && (
                <div className="rounded-xl border border-gray-100 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-800/50">
                  <p className="mb-1.5 text-xs font-medium text-gray-500 dark:text-gray-400">附加资料（当前选中的文章/笔记，将附加到提示词）</p>
                  <pre className="max-h-48 overflow-y-auto whitespace-pre-wrap text-xs leading-relaxed text-gray-600 dark:text-gray-300">{promptPreview}</pre>
                </div>
              )}
              {!systemPrompt && !promptPreview && <p className="py-6 text-center text-sm text-gray-400">暂无角色设定内容</p>}
            </div>
          ) : tab === 'site' ? (
            <div className="space-y-4">
