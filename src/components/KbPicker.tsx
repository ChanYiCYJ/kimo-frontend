...
interface KbNote { id: string; title: string; content: string; }

export function KbPicker({
  selected, onToggle, onInsert, onClose, onOpenAgent,
}: {
  selected: string[];
  onToggle: (id: string) => void; onInsert: () => void; onClose: () => void;
  onOpenAgent: () => void;
}) {
  ...
  return createPortal(
    <div className="fixed bottom-20 left-4 right-4 z-[100] mx-auto max-w-lg rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900 sm:left-16 sm:right-auto sm:w-96">
      ...
      <div className="flex items-center justify-between border-t border-gray-100 px-4 py-2 dark:border-gray-800">
        ...
        <button onClick={onInsert} disabled={selected.length === 0}
          className="rounded-lg bg-gray-900 px-4 py-1.5 text-xs font-medium text-white hover:bg-gray-700 disabled:opacity-30 dark:bg-gray-200 dark:text-gray-900">插入选中 ({selected.length})</button>
      </div>
    </div>,
    document.body,
  );
