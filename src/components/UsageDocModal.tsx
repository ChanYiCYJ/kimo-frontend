...
const S = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <section className="space-y-1.5">
    ...
    <div className="space-y-1 text-sm leading-relaxed text-gray-600 dark:text-gray-400">{children}</div>
  </section>
)

export function UsageDocModal({ open, onClose, hasCustom, canManage }: UsageDocModalProps) {
  if (!open) return null
    ...
    <div className="fixed inset-0 z-[95] flex items-center justify-center p-4">
      ...
      <div className="relative flex max-h-[88vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900">
        <div className="flex shrink-0 items-center justify-between border-b border-gray-100 px-4 py-3 dark:border-gray-700">
          <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">Kimo AI · 使用文档</h3>
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          <S title="💬 基础使用">
            <ul className="ml-4 list-disc space-y-1">
              <li>「新建会话」开始对话；侧边栏可重命名、删除、导出/导入全部会话。</li>
              <li>发送框按 Enter 发送、Shift+Enter 换行；点击左侧「＋」可上传 Markdown、切换网络搜索、打开 Coser、导出当前对话。</li>
          ...
          <S title="🎭 Coser 角色扮演">
            <ul className="ml-4 list-disc space-y-1">
              <li>Coser = 给 AI 配置"人设 + 知识"：<b>角色设定</b>（默认提示词）、<b>站点内容</b>（选择本站文章/分类喂给 AI）、<b>自定义设定</b>（本机笔记，可导入 Markdown）。</li>
              <li>所有选择保存在本机浏览器，仅自己可见；可一键「导出设定」。</li>
            </ul>
          </S>

          <S title="🌐 网络搜索">
            <ul className="ml-4 list-disc space-y-1">
              <li>在「＋」菜单开启网络搜索后，AI 会先抓取维基百科等公开内容作为上下文，回答更实时、准确。</li>
          ...
          <S title="🔑 自定义模型 API">
            <ul className="ml-4 list-disc space-y-1">
              <li>{canManage ? '管理员使用后台「AI 管理」配置的默认模型。' : '默认使用站点提供的模型；您可在侧边栏「模型 API 设置」填入自己的接口/密钥/模型，实现个性化 + 解除次数/冷却限制。'}</li>
              <li><b>安全规则：</b>密钥仅保存在本机浏览器（localStorage），不会上传服务器；请勿在公共电脑保存密钥；接口需为 OpenAI 兼容格式；如开启网络搜索/自定义 API，请遵守您所用服务的使用条款与当地法律。</li>
              <li>{hasCustom ? '当前正在使用自定义 API。' : '当前使用站点 API（可能有次数/冷却限制）。'}</li>
              {!canManage && (
                <li>使用自定义 API 时，可在「模型 API 设置」中填写自己的提示词（角色设定），覆盖默认人设。</li>
              )}
            </ul>
          </S>
          ...
          <S title="⚖️ 适用范围与合规">
            <ul className="ml-4 list-disc space-1">
              <li>管理员可在后台为每个 AI 助手开启「仅管理员可使用」；主页的「AI」菜单可在站点设置中关闭（show_ai）。</li>
              <li>AI 生成内容已加水印，请勿将 AI 内容冒充人工原创用于商业/学术等需要真实性的场合。</li>
        ...
        <div className="flex shrink-0 items-center justify-end gap-2 border-t border-gray-100 px-4 py-3 dark:border-gray-700">
          ...
          <button onClick={onClose} className="rounded-lg bg-gray-900 px-5 py-2 text-sm font-medium text-white transition hover:bg-gray-700 dark:bg-gray-200 dark:text-gray-900 dark:hover:bg-gray-300">
            完成
          </button>
        </div>
      </div>
