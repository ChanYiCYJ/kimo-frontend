...
async function streamChat(
  ...
  const sys =
    ...
    (web
      ...
      : "") +
    (browseMode
      ? `\n\n【联网浏览模式】当前已开启「浏览 Agent」，详细内容由浏览 Agent 生成文章，你只需简短引导。规则：当用户提问需要查询外部/最新/不熟悉的信息时，只允许输出 1-2 句简短的中文说明（不超过 50 字），然后必须附上 [SEARCH:关键词]；严禁自行展开成长篇回答，详细内容一律交给浏览 Agent 生成。**这 1-2 句说明请严格保持你一贯的语气与性格（按你的系统人设来回应，自然、有辨识度），不要用生硬的套话。**若问题不需要联网（寒暄、写代码、简单常识等），正常按你的风格简短回答即可。`
      : "") +
