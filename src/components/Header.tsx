    ...
    <header className="sticky top-0 z-40 border-b border-gray-200 bg-white/70 backdrop-blur dark:border-gray-800 dark:bg-gray-950/70">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <div className="flex min-w-0 items-center gap-3">
          <span className="hidden h-9 w-9 shrink-0 place-content-center overflow-hidden rounded-full bg-gray-100 sm:grid dark:bg-gray-800">
            {settings.avatar ? (
              <img
        ...
        <nav className="hidden items-center gap-4 text-sm text-gray-500 md:flex">
          ...
          {pages.some(
            (p) =>
              p.type === "html" &&
              p.status === 0 &&
              (p.content || "").startsWith(AI_CHAT_MARKER),
          ) &&
            settings.show_ai !== "0" && (
              <NavLink
                to="/ai"
          ...
          <nav className="flex flex-col gap-1 text-sm">
            ...
            {isAdmin ? (
              <Link
                to="/dashboard"
                onClick={() => setMobileOpen(false)}
                className="rounded-lg px-3 py-2 text-gray-900 hover:bg-gray-100"
              >
                管理后台
              </Link>
            ) : settings.show_dashboard !== "0" ? (
