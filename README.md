...
- **网络模式**：Fast（默认，关闭网络）/ search（联网检索）/ view（联网抓取生成综合文章）；优先走站点后端 `/api/search`（可绕过地域限制），回退维基百科（zh→en）
...
AI 的网络模式默认 **Fast（关闭网络）**。`search` 模式会联网检索，优先调用站点后端 `/api/search`（若后端实现了搜索代理，可绕过 CORS 与地域限制）；否则回退到维基百科（zh→en）。`view` 模式会在搜索后自动联网抓取并生成综合文章。由于维基百科在中国大陆不可直接访问，若你的主站在国内，建议在后端实现 `/api/search` 代理，或将主站放在海外/Vercel。
